...
template <typename T> std::string to_string(const std::optional<T> &v) {
    if (v.has_value()) {
...
inline std::ostream &operator<<(std::ostream &os, Wrap32 a) { return os << to_string(a); }

inline bool operator!=(Wrap32 a, Wrap32 b) { return not(a == b); }
