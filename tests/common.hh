...
template <>
inline ExpectationViolation::ExpectationViolation(const std::string &property_name, const bool &expected,
                                                  const bool &actual)
    : ExpectationViolation{"The object should have had " + property_name + " = " + boolstr(expected)
                           + ", but instead it was " + boolstr(actual) + "."} {}

template <class T> struct TestStep {
...
class Printer {
  ...
  public:
    ...
    void diagnostic(std::string_view test_name, const std::vector<std::pair<std::string, int>> &steps_executed,
                    const std::string &failing_step, const std::exception &e) const;
};

template <class T> class TestHarness {
...
template <class T, typename Num> struct ConstExpectNumber : public ExpectNumber<T, Num> {
...
template <class T> struct ExpectBool : public ExpectNumber<T, bool> {
    using ExpectNumber<T, bool>::ExpectNumber;
};

template <class T> struct ConstExpectBool : public ConstExpectNumber<T, bool> {
