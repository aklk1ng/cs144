...
template <std::derived_from<TestStep<Reassembler>> T> struct DirectReassemblerTest : public TestStep<TCPReceiver> {
    T step_;
...
template <std::derived_from<TestStep<ByteStream>> T> struct DirectByteStreamTest : public TestStep<TCPReceiver> {
    T step_;
...
class TCPReceiverTestHarness : public TestHarness<TCPReceiver> {
  public:
    ...
    template <std::derived_from<TestStep<Reassembler>> T> void execute(const T &test) {
