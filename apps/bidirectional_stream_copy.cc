...
void bidirectional_stream_copy(Socket &socket, string_view peer_name) {
    ...
    _eventloop.add_rule(
        ...
        [&] {
            return !_outbound.has_error() and !_inbound.has_error()
                   and (_outbound.writer().available_capacity() > 0) and !_outbound.writer().is_closed();
        },
        [&] { _outbound.writer().close(); },
        [&] {
    ...
    _eventloop.add_rule(
        ...
        [&] {
            return _outbound.reader().bytes_buffered()
                   or (_outbound.reader().is_finished() and not _outbound_shutdown);
        },
        [&] { _outbound.writer().close(); },
        [&] {
    ...
    _eventloop.add_rule(
        ...
        [&] {
            return _inbound.reader().bytes_buffered()
                   or (_inbound.reader().is_finished() and not _inbound_shutdown);
        },
        [&] { _inbound.writer().close(); },
        [&] {
