...
EventLoop::FDRule::FDRule(BasicRule &&base, FileDescriptor &&s_fd, Direction s_direction, CallbackT s_cancel,
                          CallbackT s_error)
    : BasicRule(base), fd(move(s_fd)), direction(s_direction), cancel(move(s_cancel)), error(move(s_error)) {}
...
EventLoop::RuleHandle EventLoop::add_rule(const size_t category_id, const CallbackT &callback,
...
EventLoop::Result EventLoop::wait_next_event(const int timeout_ms) {
    ...
    for (auto [it, idx] = make_pair(_fd_rules.begin(), static_cast<size_t>(0)); it != _fd_rules.end(); ++idx) {
        ...
        if (poll_ready) {
            ...
            if (count_before == this_rule.service_count() and (not this_rule.fd.closed())
                and this_rule.interest()) {
                throw runtime_error("EventLoop: busy wait detected: rule \""
                                    + _rule_categories.at(this_rule.category_id).name
