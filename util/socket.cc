...
void Socket::bind_to_device(const string_view device_name) { setsockopt(SOL_SOCKET, SO_BINDTODEVICE, device_name); }
...
void DatagramSocket::recv(Address &source_address, string &payload) {
    ...
    const ssize_t recv_len = CheckSystemCall("recvfrom", ::recvfrom(fd_num(), payload.data(), payload.size(),
...
void DatagramSocket::sendto(const Address &destination, const string_view payload) {
    CheckSystemCall("sendto",
                    ::sendto(fd_num(), payload.data(), payload.length(), 0, destination.raw(), destination.size()));
    register_write();
}
