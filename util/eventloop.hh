...
class EventLoop {
  ...
  private:
    ...
    struct FDRule : public BasicRule {
        ...
        FDRule(BasicRule &&base, FileDescriptor &&s_fd, Direction s_direction, CallbackT s_cancel,
               CallbackT s_error);
  ...
  public:
    ...
    class RuleHandle {
      ...
      public:
        template <class RuleType> explicit RuleHandle(const std::shared_ptr<RuleType> x) : rule_weak_ptr_(x) {}

        void cancel();
    };

    RuleHandle add_rule(
        size_t category_id, FileDescriptor &fd, Direction direction, const CallbackT &callback,
        const InterestT &interest = [] { return true; }, const CallbackT &cancel = [] {},
    ...
    template <typename... Targs> auto add_rule(const std::string &name, Targs &&...Fargs) {
